...
/**
 * Parse a `--timeout <seconds>` flag to milliseconds. A bare `Number()` would turn a
 * typo like `--timeout abc` into NaN, which callers read as an instantly-elapsed
 * deadline — a command that "times out" the moment it starts with no explanation.
 * Validate to a positive integer instead (mirrors --tail/--interval).
 */
