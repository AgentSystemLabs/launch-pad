...
function sampleProgram(): Command {
  const p = new Command();
  p.name("launchpad").option("--cluster <name>", "target cluster").option("--json", "json output");
...
describe("generateCompletion", () => {
  ...
  it("bash: completes top-level commands, node subcommands, and globals for both bins", () => {
    ...
    expect(out).toContain("complete -F _launch_pad_complete launchpad launch-pad lpd");
  });

  it("zsh: emits a #compdef header, _describe blocks, and a per-subcommand case arm", () => {
    const out = generateCompletion("zsh", tree, NAMES);
    expect(out).toContain("#compdef launchpad launch-pad lpd");
    expect(out).toContain("'deploy:Build and deploy the app'");
    expect(out).toContain("node)");
    expect(out).toContain("'create:Create a node'");
    expect(out).toContain("compdef _launch_pad_complete launchpad launch-pad lpd");
  });

  it("fish: emits subcommand + sub-subcommand + global-flag completes for each bin", () => {
    const out = generateCompletion("fish", tree, NAMES);
    expect(out).toContain("complete -c launchpad -n __fish_use_subcommand -a deploy");
    expect(out).toContain('complete -c launchpad -n "__fish_seen_subcommand_from node" -a create');
    expect(out).toContain("complete -c launchpad -l cluster");
