...
async function runFootprintDestroy(opts: DestroyOptions): Promise<void> {
  ...
  if (opts.wait !== false) {
    ...
    drain = await waitForDrain(
      ...
      ownerProject,
      removeSet,
      parseTimeoutMs(opts.timeout, DEFAULT_DRAIN_TIMEOUT_SECONDS, "pass whole seconds ≥ 1, e.g. --timeout 120"),
    );
...
async function runEnvDestroy(env: string, opts: DestroyOptions): Promise<void> {
  ...
  try {
    report = await destroyEnvFootprint(aws, marker, {
      ...
      timeoutMs: parseTimeoutMs(opts.timeout, DEFAULT_DRAIN_TIMEOUT_SECONDS, "pass whole seconds ≥ 1, e.g. --timeout 120"),
...
async function runProjectDestroy(project: string, opts: DestroyOptions): Promise<void> {
  ...
  const timeoutMs = parseTimeoutMs(opts.timeout, DEFAULT_DRAIN_TIMEOUT_SECONDS, "pass whole seconds ≥ 1, e.g. --timeout 120");
